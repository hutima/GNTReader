...
// --- FL-008 helpers (iPad panel-reflow jump + visible-chapter tracking) ---
 ...
 * Measuring drift before that settles would blame FL-008 for FL-004/network
 ...
 * (FL-008), and even a single token's exact line placement can shift a few
...
try {
  ...
  // 10a. FL-008 width-anchor: 768×1024 (iPad mini portrait) — the desktop
  ...
  // 10b. FL-008 width-anchor at 834×1112 (iPad Air/Pro portrait): one cycle.
  {
    const ctx = await browser.newContext({
  ...
  // 10c. FL-008 width-anchor under rotation: 768×1024 → 1024×768 with the
  // panel already open (page.setViewportSize simulates the iPad rotating).
  {
    ...
    // Capture the reference verse BEFORE opening the panel. FL-008's contract
    ...
    // two adjacent verses". That gap WAS the FL-008 anchor-flip bug (see
    // failure-log FL-008): consecutive `.verse` spans share the line where one
  ...
  // 10d. Visible-chapter tracking (FL-008): scroll slowly from John 1 into
